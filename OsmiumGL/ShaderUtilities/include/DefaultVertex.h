...
struct DefaultVertex {
    ...
    static std::array<VkVertexInputAttributeDescription, 4> getAttributeDescriptions() {
        ...
        attributeDescriptions[1].offset = offsetof(DefaultVertex,texCoordinates);

        attributeDescriptions[2].binding = 0;
        attributeDescriptions[2].location = 2;
        attributeDescriptions[2].format = VK_FORMAT_R32G32_SFLOAT;
        attributeDescriptions[2].offset = offsetof(DefaultVertex, normal);
...
namespace std {
    template <> struct hash<DefaultVertex> {
        size_t operator()(const DefaultVertex& t) const noexcept {
            return ((hash<glm::vec3>()(t.position) ^ (hash<glm::vec3>()(t.normal) << 1)) >> 1) ^
